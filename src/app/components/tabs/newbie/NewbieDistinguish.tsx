...
const oceanPresets: Record<string, FloatSeries[]> = {
    Atlantic: [
        ...
        { id: "AT-12345 (Cold Front)", temps: [19, 18, 16, 14, 11, 8, 6, 5], salinity: [34.8, 34.9, 35.0, 35.1, 35.0, 34.9, 34.8, 34.7], humidity: [92, 90, 88, 85, 83, 80, 78, 75], color: "#1E90FF", emoji: "❄" },
    ...
    Pacific: [
        ...
        { id: "PX-20456 (Upwelling)", temps: [16, 15, 14, 12, 10, 8, 6, 5], salinity: [33.8, 33.9, 34.0, 34.1, 34.2, 34.3, 34.4, 34.5], humidity: [95, 93, 91, 89, 87, 85, 83, 81], color: "#1E90FF", emoji: "❄" },
    ...
    Indian: [
        ...
        { id: "IN-66778 (Cool Patch)", temps: [18, 17, 15, 13, 11, 9, 7, 6], salinity: [35.2, 35.3, 35.4, 35.3, 35.2, 35.1, 35.0, 34.9], humidity: [93, 91, 89, 86, 84, 81, 79, 77], color: "#1E90FF", emoji: "❄" },
    ...
    Southern: [
        { id: "SO-00123 (Cold Core)", temps: [10, 9, 8, 7, 6, 5, 4, 3], salinity: [33.9, 34.0, 34.1, 34.2, 34.3, 34.4, 34.5, 34.6], humidity: [98, 96, 94, 92, 90, 88, 86, 84], color: "#1E90FF", emoji: "❄" },
    ...
    Arctic: [
        { id: "AR-90001 (Cold Surface)", temps: [4, 3.8, 3.5, 3.2, 3.0, 2.8, 2.6, 2.5], salinity: [32.0, 32.1, 32.2, 32.3, 32.4, 32.5, 32.6, 32.7], humidity: [99, 98, 97, 96, 95, 94, 93, 92], color: "#1E90FF", emoji: "❄" },
...
const KeyTakeawaysPanel = ({ stats, visibleIds, dataType }: { stats: any[]; visibleIds: string[]; dataType: DataType; }) => {
    ...
    const warmFloat = visibleStats.find(s => s.type.includes('Warm') || s.type.includes('High'));
    const coldFloat = visibleStats.find(s => s.type.includes('Cold') || s.type.includes('Low'));

    const getDescription = (type: DataType) => {
        switch (type) {
            case 'Temperature':
                return {
                    warmTitle: 'Warm Core Anomaly',
                    warmDescription: warmFloat ? `Indicates a potential eddy or unusual surface heating. Float ${warmFloat.id.split(' ')[0]} is the primary subject for this event.` : 'Indicates a potential eddy or unusual surface heating.',
                    coldTitle: 'Cold Intrusion',
                    coldDescription: coldFloat ? `Suggests upwelling or the influence of a cold front, observed in float ${coldFloat.id.split(' ')[0]}.` : 'Suggests upwelling or the influence of a cold front.',
                };
            case 'Salinity':
                return {
                    warmTitle: 'High Salinity Anomaly',
                    warmDescription: warmFloat ? `Indicates a region with high surface salinity, potentially due to evaporation or a specific current. Float ${warmFloat.id.split(' ')[0]} is the primary subject for this event.` : 'Indicates a region with high surface salinity.',
                    coldTitle: 'Low Salinity Intrusion',
                    coldDescription: coldFloat ? `Suggests the influence of freshwater inflow, such as river runoff or ice melt, observed in float ${coldFloat.id.split(' ')[0]}.` : 'Suggests the influence of freshwater inflow.',
                };
            case 'Humidity':
                return {
                    warmTitle: 'Low Humidity Anomaly',
                    warmDescription: warmFloat ? `Indicates an area with unusually dry air, possibly linked to a descending air mass. Float ${warmFloat.id.split(' ')[0]} is the primary subject for this event.` : 'Indicates an area with unusually dry air.',
                    coldTitle: 'High Humidity Intrusion',
                    coldDescription: coldFloat ? `Suggests the presence of a moist air mass or local evaporation, observed in float ${coldFloat.id.split(' ')[0]}.` : 'Suggests the presence of a moist air mass or local evaporation.',
                };
            default:
                return {
                    warmTitle: 'Warm Core Anomaly',
                    warmDescription: 'Indicates a potential eddy or unusual surface heating.',
                    coldTitle: 'Cold Intrusion',
                    coldDescription: 'Suggests upwelling or the influence of a cold front.',
                };
        }
    };
    
    const descriptions = getDescription(dataType);
                ...
                <div className="text-sm">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 font-medium text-foreground">
                            ...
                            <span>{descriptions.warmTitle}</span>
                        </div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground pl-6">{descriptions.warmDescription}</p>
                </div>
                <div className="text-sm">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 font-medium text-foreground">
                            <Info className="h-4 w-4 text-blue-400 shrink-0"/>
                            <span>{descriptions.coldTitle}</span>
                        </div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground pl-6">{descriptions.coldDescription}</p>
                </div>
                <div className="text-sm">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 font-medium text-foreground">
...
const componentStyles = `
    ...
    .component-scroller::-webkit-scrollbar-track {
        background: transparent;
    }
    .component-scroller::-webkit-scrollbar-thumb {
    ...
    /* Remove body scrollbar to prevent double scrollbars */
    html, body {
        overflow-y: hidden;
    }
    .toggle {
...
export default function ResearcherCompare({ theme = "light", floats: floatsProp, depths: depthsProp }: Props) {
    ...
    const downloadCSV = () => {
        const unit = getUnit(dataType);
        const dataToExport = floatsFromPreset.filter(f => visibleIds.includes(f.id));
        const headers = [
            'Float ID',
            'Type',
            `Surface (${unit})`,
            ...
            `Max (${unit})`,
            `Mean (${unit})`,
            ...depths.filter(d => d >= depthRange.min && d <= depthRange.max).map(d => `${d}m (${unit})`)
        ];

        const csvRows = dataToExport.map(float => {
            const stat = stats.find(s => s.id === float.id);
            if (!stat) return '';
            ...
            const row = [
                ...
                ...depths
                    .map((depth, index) => ({ depth, index }))
                    .filter(({ depth }) => depth >= depthRange.min && depth <= depthRange.max)
                    .map(({ index }) => (float[dataKey] as number[])[index])
            ];
            return row.join(',');
        });
        ...
        link.setAttribute('download', `compare_${dataType.toLowerCase()}.csv`);
    ...
    return (
        <div className="min-h-screen font-sans">
            ...
            `}</style>
            <div className="relative h-screen overflow-y-scroll overflow-x-hidden component-scroller p-4 sm:p-6">
                <div ref={tabRef} className="max-w-8xl mx-auto space-y-6">
                    <div>
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="leading-tight">
                                                    <div className="font-medium text-xs text-foreground flex items-center gap-1">
                                                        <span>{getDescriptiveEmoji(s.type, dataType)}</span>
                                                        <span>{s.id.split(' ')[0]}</span>
                                                        <span className="text-muted-foreground font-normal">{getDescriptiveType(s.type, dataType)}</span>
                                                    </div>
